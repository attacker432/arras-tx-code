...
const suggestionChannels = config.channels.suggestionchannel
const logChannels = new Keyv(process.env.logChannels);
const msgLogs = new Keyv(process.env.msgLogs);
const welcomeChannels = config.gatewayChannel;
const leaveChannels = config.gatewayChannel;
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let suggestionChannel = config.channels.suggestionChannel;
   ...
   /* let welcomeChannel = await welcomeChannels.get(`welcomechannel_${interaction.guild.id}`);
    ...
    const botSettingsEmbed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Suggestion Channel', value: '```' + suggestionChannel+"(#suggestions)" + '```', inline: true },
        { name: 'Log Channel', value: '```' + `${logChannel} [${logChannelState}]` + '```', inline: true },
        { name: 'Welcome Channel', value: '```' + welcomeChannels + '(#gateway)```', inline: true },
        ...
        { name: 'Leave Channel', value: '```' + leaveChannels + '(#gateway)```', inline: true },
