...
const config = require('../../server_data.js')
module.exports = async (client, member) => {
  const welcomeChName = await welcomeChannels.get(
  ...
  if (dm && dmState == 1)
    member.send({ content: dm.replace("[user]", member.user.username) });

  const welcomeRoleName = await welcomeRoles.get(
    `welcomerole_${member.guild.id}`
  );
  const welcomeRole = member.guild.roles.cache.find(
    (role) => role.name === `Member`
  );
//  let state = await toggleWelcome.get(`togglewelcomemsg_${member.guild.id}`);
let state = config.server_settings.toggle_welcome_msg;
